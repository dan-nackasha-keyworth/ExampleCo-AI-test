...
def classify_and_extract(client, message_text, config, entry_channel=None):
    ...
    system_prompt = (
        ...
        f"message. This rule is specifically about sensitive_topic_flags "
        f"(overcharge/rate dispute) - it does not by itself make "
        f"retention_risk_language true. Objecting to a charge as unfair is "
        f"a different question from expressing intent to leave or "
        f"reconsider the relationship (see retention_risk_language above); "
        f"a message can be angry about a charge without containing any "
        f"leaving/switching/cancelling language at all, and the two fields "
        f"should be scored independently of each other.\n\n"
